...
freq <- seq(1,nrow(sh$mtm$eigenCoefs),2) # frequencies at which to calculate the transfer function
...
F_test_A <- matrix(ncol = length(freq), nrow = 7) # initialize empty matrix for sampling at given frequencies
...
for(j in 1:length(freq)){  
  ...
  F_test_A[,j] <- t(s$mtm$eigenCoefs[freq[j],]) #take row corresponding to jth chosen frequency
}
...
#convert prediction to a useful object -----------

#assume F-test from training data applies to prediction
ftest <- s$mtm$Ftest
k=7
freqIndex <- test_function(ftest, k, 0.9)
